document.addEventListener('DOMContentLoaded', () => {
  ...
  const switchPage = (pageId, scrollTargetId) => {
    ...
    document.querySelectorAll('.nav-links .nav-link').forEach(link => {
        ...
        if (linkPage === pageId && !link.dataset.scroll) {
    ...
    if (pageId !== 'home') {
        ...
        header.classList.toggle('light-bg', window.scrollY > 50);
    }
  ...
  if (iframe) {
    ...
    // Ensure autoplay is not in the original src to prevent issues
    originalIframeSrc = originalIframeSrc.replace('&autoplay=1', '').replace('?autoplay=1', '');
  }

  const openModal = () => {
    if (videoModal && iframe) {
      videoModal.style.display = 'flex';
      // Add autoplay parameter intelligently
      if (originalIframeSrc.includes('?')) {
        iframe.src = originalIframeSrc + '&autoplay=1';
      } else {
        iframe.src = originalIframeSrc + '?autoplay=1';
      }
    }
  ...
  if(videoModal) videoModal.addEventListener('click', (e) => e.target === videoModal && closeModal());
  document.addEventListener('keydown', (e) => e.key === 'Escape' && videoModal.style.display === 'flex' && closeModal());
  ...
  if (newHeroSection) {
      ...
      const updateProductDisplay = (productKey) => {
          ...
          productNameEl.parentElement.style.opacity = '0';
          productNameEl.parentElement.style.transform = 'translate(-50%, -10%)';
          ...
          setTimeout(() => {
              ...
              productNameEl.parentElement.style.opacity = '1';
              productNameEl.parentElement.style.transform = 'translate(-50%, -20%)';
          }, 300); 
      };
